...
public enum GroupErrorCode implements ErrorCode {
    ...
    IMAGE_UPLOAD_EXCEED(HttpStatus.BAD_REQUEST,
            "모임: 모임 이미지는 최대 3개까지 가능합니다. 현재 이미지 수: %s"),
    IMAGE_UPLOAD_FAILED(HttpStatus.BAD_REQUEST,
            "모임: 이미지 업로드 요청에 실패했습니다: %s"),
    IMAGE_URL_REQUIRED(HttpStatus.BAD_REQUEST, "모임: 모임 이미지 삭제 url은 필수입니다."),
    GROUP_IMAGE_NOT_FOUND(HttpStatus.NOT_FOUND, "모임 이미지가 존재하지 않습니다. groupId=%d"),
    GROUP_IMAGE_KEY_REQUIRED(HttpStatus.BAD_REQUEST,
            "모임: 이미지 키는 필수입니다."),
    GROUP_IMAGE_MUST_440_240(HttpStatus.BAD_REQUEST,
            "모임: 모임 이미지 440x240은 필수입니다."),
    ...
    GROUP_IMAGE_NOT_FOUND_IN_GROUP_AFTER_UPDATE(HttpStatus.BAD_REQUEST,
    ...
    TAG_DUPLICATED(HttpStatus.BAD_REQUEST, "모임: 동일한 태그는 중복으로 추가할 수 없습니다."),
    ...
    MAX_PARTICIPANTS_BELOW_ATTEND_COUNT(HttpStatus.CONFLICT,
            "모임: 현재 참석자 수(%s)보다 정원을 줄일 수 없습니다."),
    INVALID_MAX_PARTICIPANTS_LESS_THAN_CURRENT(HttpStatus.BAD_REQUEST,
            "모임: 현재 참여 인원 수(%s)보다 작은 값으로 최대 인원을 줄일 수 없습니다."),

    // 수정 / 삭제 권한
    GROUP_ONLY_HOST_CAN_UPDATE(HttpStatus.FORBIDDEN, "모임: 호스트만 수정할 수 있습니다."),
    GROUP_CANNOT_UPDATE_IN_STATUS(HttpStatus.CONFLICT,
            "모임: 현재 상태(%s)에서는 수정할 수 없습니다."),
    NO_PERMISSION_TO_UPDATE_GROUP(HttpStatus.FORBIDDEN,
            "모임: 해당 모임을 수정할 권한이 없습니다. 모임 ID: %s 회원 ID: %s"),
    NO_PERMISSION_TO_DELETE_GROUP(HttpStatus.UNAUTHORIZED,
            "모임: 삭제할 수 있는 권한이 없습니다."),
    ...
    GROUP_NOT_RECRUITING(HttpStatus.BAD_REQUEST,
            "모임: 모집 상태가 아닙니다. 현재 상태: %s"),
    GROUP_NOT_ATTEND_STATUS(HttpStatus.BAD_REQUEST,
            "모임: ATTEND 상태에서만 LEFT 상태로 전이할 수 있습니다."),

    // Redis / 선 업로드 이미지
    PRE_UPLOADED_IMAGE_NOT_FOUND(HttpStatus.NOT_FOUND,
            "모임: 선 업로드 이미지가 만료되었거나 존재하지 않습니다."),
    PRE_UPLOADED_IMAGE_OWNER_MISMATCH(HttpStatus.FORBIDDEN,
            "모임: 선 업로드 이미지를 업로드한 사용자만 사용할 수 있습니다."),
    REDIS_SAVE_FAILED(HttpStatus.INTERNAL_SERVER_ERROR,
            "모임: 선 업로드 이미지 저장에 실패했습니다."),
    REDIS_READ_FAILED(HttpStatus.INTERNAL_SERVER_ERROR,
            "모임: 선 업로드 이미지 조회에 실패했습니다."),

    // 조회 / 존재 여부
    GROUP_NOT_FOUND_BY_ID(HttpStatus.NOT_FOUND,
            "모임: 모임을 찾을 수 없습니다. 모임 ID: %s"),
    GROUP_DELETED(HttpStatus.NOT_FOUND,
            "모임: 삭제된 모임입니다."),
    GROUP_USER_NOT_FOUND(HttpStatus.NOT_FOUND,
            "모임: 회원을 찾을 수 없습니다. 회원 ID: %s"),
    HOST_USER_NOT_FOUND(HttpStatus.NOT_FOUND,
            "모임: 호스트 사용자를 찾을 수 없습니다: %s"),
    USER_ID_NULL(HttpStatus.NOT_FOUND,
            "모임: 회원 ID가 null 입니다."),
    ...
    GROUP_HOST_CANNOT_LEAVE(HttpStatus.BAD_REQUEST,
            "모임: HOST는 나갈 수 없습니다. 모임 ID: %s 회원 ID: %s"),
    GROUP_HOST_CANNOT_ATTEND(HttpStatus.BAD_REQUEST,
            "모임: HOST는 다시 모임에 신청을 할 수 없습니다."),
    NOT_ATTEND_GROUP(HttpStatus.BAD_REQUEST,
            "모임: 참여한 적 없거나 이미 나간 상태입니다. 모임 ID: %s 회원 ID: %s"),
    ALREADY_ATTEND_GROUP(HttpStatus.BAD_REQUEST,
            "모임: 이미 참여 중인 모임입니다. 모임 ID: %s 회원 ID: %s"),
    GROUP_IMAGE_SORT_ORDER_CONFLICT(HttpStatus.CONFLICT,
